...
def generate_same_pairs(index_directory: str, others_directory: str,
    ...
    """
    # Create s folder for all same paires
    same_output_directory = output_directory + '/same'
    # Don't create the file if it already exists
    if not os.path.exists(same_output_directory):
        os.makedirs(same_output_directory)
        ...
        if same_images:
            ...
            for i in range(0, len(same_images)):
                for j in range(i+1, len(same_images)):
                    ...
                    same_dir = same_output_directory + '/same' + same_images[i] + '-' + same_images[j]
...
def generate_different_pairs(num_same_paires: int, index_directory: str,
    ...
    """
    # Create s folder for all different paires
    different_output_directory = output_directory + '/different'
    # Don't create the file if it already exists
    if not os.path.exists(different_output_directory):
        os.makedirs(different_output_directory)
    ...
    while num_different_pairs < num_same_paires:
        ...
        if index_id != others_id:
            different_dir = different_output_directory + '/different' + index_id + '-' + others_name
